...
`);
  ...
  // Log annotation to markdown file
  async function logToMarkdown(annotation) {
    ...
    try {
      ...
      // Format annotation as markdown entry
      const date = new Date(annotation.timestamp).toLocaleString();
      const status = getStatusEmoji(annotation.status);
      const priority = '🔥'; // Could be dynamic based on annotation type
      
      const markdownEntry = `
## ${status} ${annotation.elementLabel}
...
${annotation.screenshot ? '![Element Screenshot](data:image/png;base64,...)' : ''}
...
`;
      ...
      if (!existingContent.trim()) {
        console.log('Moat: File is empty, adding header...');
        newContent = `# Moat Tasks\n\nGenerated by Moat Chrome Extension\n\n`;
      }
      
      // Insert new entry after header (find first "---" or add after header)
