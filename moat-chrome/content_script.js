  ...
  async function checkDeployedTemplates() {
    ...
    try {
      ...
      try {
        ...
        const isFallback = workflowContent.includes('Drawbridge Workflow - Complete Rules (Fallback)');
        ...
        results.workflow = {
          ...
          version: isFallback ? 'FALLBACK' : 'FULL'
        };
      ...
      try {
        ...
        const isFallback = readmeContent.includes('This is a basic template - the full README');
      ...
      // Overall status
      const hasFallbacks = results.workflow?.isFallback || results.readme?.isFallback;
      const status = hasFallbacks ? '⚠️ USING FALLBACKS' : '✅ FULL TEMPLATES';
      ...
      return { success: true, results, status, hasFallbacks };
