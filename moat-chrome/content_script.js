...
`);
  ...
  // Log annotation to markdown file with enhanced format
  async function logToMarkdown(annotation) {
    ...
    try {
      console.log('Moat: Reading existing markdown content...');
      // Read existing content
      const file = await markdownFileHandle.getFile();
      const existingContent = await file.text();
      console.log('Moat: Existing content length:', existingContent.length);
...
`;
      ...
      console.log('Moat: Creating markdown entry...', {
        elementLabel: annotation.elementLabel,
        content: annotation.content,
        status: annotation.status
      });
      
      // Write back with new entry at the top (after header)
      console.log('Moat: Creating writable stream...');
      const writable = await markdownFileHandle.createWritable();
      
      // If file is empty, add enhanced header
      let newContent = existingContent;
      if (!existingContent.trim()) {
        console.log('Moat: File is empty, adding enhanced header...');
        const url = new URL(annotation.pageUrl);
        const projectName = url.hostname === 'localhost' ? 'Local Development' : url.hostname;
        
        newContent = `# Moat Tasks - ${projectName}
...
`;
        ...
        // Update the header with current task count if enhanced header exists
        const existingTasks = parseExistingTasks(existingContent);
        const headerPattern = /(\*\*Total Tasks\*\*): (\d+)/;
        const lastUpdatedPattern = /(\*\*Last Updated\*\*): ([^\n]+)/;
        
        if (headerPattern.test(newContent)) {
          newContent = newContent.replace(headerPattern, `$1: ${existingTasks.length + 1}`);
          newContent = newContent.replace(lastUpdatedPattern, `$1: ${new Date().toISOString()}`);
        }
      }
      
      // Insert new entry after header (find first "---" or add after existing header)
      let headerEnd = newContent.indexOf('---\n');
      if (headerEnd !== -1) {
        headerEnd += 4; // After "---\n"
        newContent = newContent.slice(0, headerEnd) + '\n' + markdownEntry + newContent.slice(headerEnd);
      } else {
        // Fallback for simple header format
        headerEnd = newContent.indexOf('\n\n') + 2;
        newContent = newContent.slice(0, headerEnd) + markdownEntry + newContent.slice(headerEnd);
      }
      
      console.log('Moat: Writing updated content to file...');
      console.log('Moat: New content length:', newContent.length);
      
      await writable.write(newContent);
      await writable.close();
      
      console.log('Moat: Markdown file updated successfully');
      return true;
    } catch (error) {
      console.error('Moat: Failed to log to markdown', error);
      
      // If we lost permission, notify user
      if (error.name === 'NotAllowedError' || error.name === 'SecurityError') {
        console.warn('Moat: Lost permission to markdown file, clearing handle');
        markdownFileHandle = null;
        showNotification('Lost access to markdown file. Please reconnect project.', 'error');
      }
