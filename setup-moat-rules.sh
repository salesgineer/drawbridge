...
echo "🚀 Setting up Float automation rules for Cursor..."

# Create .cursor/rules directory if it doesn't exist
mkdir -p .cursor/rules

# Create or append to project-rules.mdc
if [ -f .cursor/rules/project-rules.mdc ]; then
    echo "" >> .cursor/rules/project-rules.mdc
    echo "---" >> .cursor/rules/project-rules.mdc
    echo "" >> .cursor/rules/project-rules.mdc
fi

# Add Float automation rules
cat >> .cursor/rules/project-rules.mdc << 'RULES'
...
- "move to bottom" → Add `position: fixed; bottom: [value];` or Tailwind `fixed bottom-[value]`
- "center horizontally" → Add `margin: 0 auto;` or Tailwind `mx-auto`
- "align right" → Add `margin-left: auto;` or Tailwind `ml-auto`
- "16px spacing" → Use `1rem` or Tailwind `4` (16px = 1rem = space-4)

### Style Changes
- Color requests → Update background/text color classes
- Size changes → Adjust width/height/padding
- Typography → Update font size/weight/family

### Component Updates
- Text changes → Update the component content directly
- Add elements → Insert new JSX/HTML
- Remove elements → Comment out or delete with explanation
...
- Confirm: "✓ Moved Profile link to bottom with 16px spacing"
...
echo "✅ Float rules added to .cursor/rules/project-rules.mdc"
echo ""
echo "Next steps:"
echo "1. Restart Cursor to load the new rules"
echo "2. Type 'watch float' in Cursor chat"
echo "3. Create annotations in Float - they'll be processed automatically!" 